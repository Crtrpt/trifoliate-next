...
class Ctx {
  ...
  expandContainer(p, ctx) {
    ...
    ctx.hashIds.get(p.id).attr["isExpand"] = !ctx.hashIds.get(p.id).attr[
      "isExpand"
    ];
  ...
  lockContainer(p, ctx) {
    ...
    ctx.hashIds.get(p.id).attr["isLock"] = !ctx.hashIds.get(p.id).attr[
      "isLock"
    ];
  }
